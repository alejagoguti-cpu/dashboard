...
function TweetCell({ tweet, onOpen, onDelete }) {
  return (
    <div
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 mb-1.5">
            <span className="text-xs font-semibold text-slate-900">
              {formatDayLabel(tweet.at)} · {formatTime(tweet.at)}
            </span>
          </div>
          <p className="text-xs text-slate-600 line-clamp-2">{tweet.title || tweet.content}</p>
          {tweet.stats && (
            <div className="flex items-center gap-3 text-[10px] text-slate-400 mt-1.5">
              {tweet.stats.map((stat) => (
  ...
  return (
    <>
      ...
      >
        ...
        <span className="text-xs font-medium text-slate-700 mt-2">Nuevo Hilo</span>
...
export default function TwitterFeedPlanner({ onSchedule }) {
  ...
  function handleFiles(fileList) {
    ...
    onSchedule({ platform: 'twitter', preview: true })
  }
    ...
    <div className="bg-white rounded-xl border border-slate-200/80 shadow-xs overflow-hidden">
      <div className="p-4 px-5 border-b border-slate-100 flex items-center justify-between gap-3 flex-wrap bg-white">
        <div className="flex items-center gap-2">
          <h2 className="text-sm font-semibold text-slate-900">Feed de Hilos</h2>
        ...
        <button
          ...
          className="px-3 py-1.5 text-xs font-medium text-white bg-slate-900 hover:bg-slate-800 rounded-lg transition flex items-center gap-1.5"
        >
      ...
      <div className="p-4 space-y-2 max-h-96 overflow-y-auto">
        {twitterPosts.length === 0 ? (
          <div className="p-6 text-center">
            <p className="text-xs text-slate-500 mb-3">Los hilos que programes aparecerán aquí</p>
