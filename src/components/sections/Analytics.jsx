...
export default function Analytics() {
  ...
  return <>
    ...
    <div className="grid xl:grid-cols-3 gap-5"><section className={`${card} premium-card xl:col-span-2 p-6`}><div className="flex items-start justify-between"><div><h2 className="panel-title">Evolución semanal</h2><p className="panel-subtitle">Miles de personas alcanzadas · misma escala durante todo el periodo</p></div><span className="live-pill"><i/> +{Math.round((series[network].at(-1)/series[network][0]-1)*100)}%</span></div><TrendChart values={series[network]} color={active.color}/></section><section className={`${card} premium-card p-6`}><h2 className="panel-title">Distribución por canal</h2><p className="panel-subtitle">Porcentaje del alcance total del periodo</p><div className="donut"><div><strong>946K</strong><span>alcance</span></div></div><div className="space-y-3 mt-5">{networks.slice(1).map((item,i)=><div key={item.id} className="flex items-center justify-between text-xs"><span className="flex items-center gap-2"><i className="w-2 h-2 rounded-full" style={{background:item.color}}/>{item.label}</span><div className="flex items-center gap-3"><span className="text-slate-400">{['197K','384K','219K','146K'][i]}</span><strong>{[21,40,23,16][i]}%</strong></div></div>)}</div></section></div>
