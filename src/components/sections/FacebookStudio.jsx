...
export default function FacebookStudio({ onSchedule }) {
  const { posts, removePost, facebook, setSection } = useDashboard()
  const [detail, setDetail] = useState(null)
  const platform = platforms.facebook

  const live = facebook ?? null
  ...
  const recentPosts = live?.posts?.length ? live.posts : platformRecentPosts.facebook
  ...
  return (
    <>
        ...
        <div className={`${card} p-5 space-y-4`}>
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                Alcance, interacción y rendimiento de cada publicación
              </p>
            </div>
            {showingDemo && (
      ...
      <Modal
        ...
        title={detail?.title}
        subtitle="Contenido y análisis de la publicación"
        width="max-w-3xl"
        footer={detail && (
          <>
            {detail.permalink && (
            ...
            <button
              ...
              onClick={() => {
                setDetail(null)
                onSchedule({ platform: 'facebook', title: detail.title, content: detail.adaptedScript })
              }}
        ...
        {detail && (
          <div className="space-y-4">
            ...
            <div className="rounded-lg border border-cyan-100 bg-cyan-50/40 p-4">
              <p className="text-[10px] font-semibold uppercase tracking-wide text-cyan-600">Adaptado para Facebook</p>
              <p className="text-xs text-slate-700 mt-1.5 leading-relaxed">{detail.adaptedScript || 'Análisis para adaptar a tu marca y comunidad.'}</p>
            </div>
          </div>
        )}
      </Modal>
